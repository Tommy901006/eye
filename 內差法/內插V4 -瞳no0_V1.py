    ...
    if file_name.endswith('.xlsx') and not file_name.startswith('~$'):
        ...
        try:
            ...
            if all(col in df.columns for col in ['Time', 'Pupil Diameter Left (mm)', 'Pupil Diameter Right (mm)']):
                ...
                # 如果第一筆資料的左右眼瞳孔數據為0，替換為3
                if df_selected.iloc[0]['Pupil Diameter Left (mm)'] <=  0:
                    df_selected.at[df_selected.index[0], 'Pupil Diameter Left (mm)'] = 3
                if df_selected.iloc[0]['Pupil Diameter Right (mm)'] <=  0:
                    df_selected.at[df_selected.index[0], 'Pupil Diameter Right (mm)'] = 3
                
                # 如果最後一筆資料的左右眼瞳孔數據為0，替換為3
                if df_selected.iloc[-1]['Pupil Diameter Left (mm)'] <= 0:
                    df_selected.at[df_selected.index[-1], 'Pupil Diameter Left (mm)'] = 3
                if df_selected.iloc[-1]['Pupil Diameter Right (mm)'] <= 0:
                    df_selected.at[df_selected.index[-1], 'Pupil Diameter Right (mm)'] = 3
