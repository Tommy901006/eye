...
source_folder = r'C:\Users\User\Desktop\post\1'  # 替換為源資料夾的路徑
target_folder = r'C:\Users\user\Desktop\2'  # 替換為目標資料夾的路徑
        ...
        try:
            ...
            if all(col in df.columns for col in ['Time', 'Pupil Diameter Left (mm)', 'Pupil Diameter Right (mm)']):
                ...
                for i in range(len(df_selected) - 1):
                    ...
                    x1, y1, y1_y = row1['Time'], row1['Pupil Diameter Left (mm)'], row1['Pupil Diameter Right (mm)']
                    x2, y2, y2_y = row2['Time'], row2['Pupil Diameter Left (mm)'], row2['Pupil Diameter Right (mm)']
